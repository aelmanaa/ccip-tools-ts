...
export class EVMChain extends Chain<typeof ChainFamily.EVM> {
  ...
  override async checkExecute(opts: {
    ...
    // nothing pool-related to check for data-only messages (mirrors the base layer)
    if (!message.tokenAmounts?.length) return true

    // The OffRamp's (memoized) version drives both the lane gates below and the releaseOrMint
    // dispatch in the token loop; tolerate failure (probes/best-effort paths then decide).
    const offRampVersion = (await this.typeAndVersion(offRamp).catch(() => undefined))?.[1]

    // OffRamp lane gates first — definitive on-chain conditions the pool-direct simulation
    // bypasses (it never enters the OffRamp): source lane enabled, and the sending OnRamp among
    // the OffRamp's allowed onRamps. Only fetched for >=1.6 OffRamps, where it's two cheap view
    // reads (`getSourceChainConfig`): 1.2/1.5 configs carry no `isEnabled` (lane existence was
    // already proven by resolving this offRamp, and the onRamp<->offRamp pairing is enforced at
    // discovery) while their config read drags whole static/dynamic/CommitStore configs.
    const offRampConfig =
      offRampVersion !== undefined && offRampVersion >= CCIPVersion.V1_6
        ? await this.getOffRampConfig(offRamp, message.sourceChainSelector).catch(
            (err: unknown) => {
              // typed wrong-lane verdicts pass through; only transport failures are best-effort
              if (err instanceof CCIPSourceChainUnsupportedError) throw err
              return undefined // transport failures don't outweigh the sim's own checks
            },
          )
        : undefined
    if (offRampConfig) {
